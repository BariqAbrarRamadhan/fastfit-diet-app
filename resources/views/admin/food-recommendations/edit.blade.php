...
@section('title', 'Edit Rekomendasi Makanan')
...
<div class="p-6">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Edit Rekomendasi Makanan</h1>
                    <p class="text-gray-600">Perbarui informasi rekomendasi makanan: {{ $foodRecommendation->name }}</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <div class="flex items-center justify-center">
            <img src="{{ asset('storage/' . $foodRecommendation->image) }}" 
                 ...
                 class="max-w-md h-48 object-cover rounded-lg shadow">
        </div>
    </div>
    @endif
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="p-6">
            <form action="{{ route('admin.food-recommendations.update', $foodRecommendation) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   id="name"
                                   name="name" 
                                   value="{{ old('name', $foodRecommendation->name) }}"
                    ...
                    <div class="mt-4">
                        ...
                        <textarea id="description" 
                                  ...
                                  required>{{ old('description', $foodRecommendation->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                                ...
                                @foreach($dietTypes as $key => $value)
                                    <label class="flex items-center p-2 rounded hover:bg-gray-50 transition-colors cursor-pointer">
                                        <input type="checkbox" 
                                               name="diet_types[]" 
                                               value="{{ $key }}"
                                               {{ (collect(old('diet_types', $foodRecommendation->diet_types))->contains($key)) ? 'checked' : '' }}
                        ...
                        <div>
                            ...
                            <select id="meal_type"
                                ...
                                @foreach($mealTypes as $key => $value)
                                    <option value="{{ $key }}" {{ old('meal_type', $foodRecommendation->meal_type) === $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="day"
                                ...
                                @foreach($dayTypes as $key => $value)
                                    <option value="{{ $key }}" {{ old('day', $foodRecommendation->day) === $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" 
                                   ...
                                   value="{{ old('calories_per_serving', $foodRecommendation->calories_per_serving) }}"
                ...
                <div class="border-t border-gray-200 pt-6">
                    ...
                    <label class="flex items-center p-3 bg-gray-50 rounded-lg cursor-pointer">
                        <input type="checkbox" 
                               name="is_active" 
                               value="1"
                               {{ old('is_active', $foodRecommendation->is_active) ? 'checked' : '' }}
